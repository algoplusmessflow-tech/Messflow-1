-- =====================================================
-- MESSFLOW — FINAL COMBINED MIGRATION
-- Date: 2026-03-20
...
-- meal_type / rice_type / dietary_preference as TEXT
DO $$ BEGIN ALTER TABLE public.members ADD COLUMN meal_type TEXT DEFAULT 'both';
EXCEPTION WHEN duplicate_column THEN
  BEGIN ALTER TABLE public.members ALTER COLUMN meal_type TYPE TEXT USING meal_type::TEXT;
  EXCEPTION WHEN others THEN NULL; END;
END $$;

DO $$ BEGIN ALTER TABLE public.members ADD COLUMN rice_type TEXT DEFAULT 'white_rice';
EXCEPTION WHEN duplicate_column THEN
  BEGIN ALTER TABLE public.members ALTER COLUMN rice_type TYPE TEXT USING rice_type::TEXT;
  EXCEPTION WHEN others THEN NULL; END;
END $$;

DO $$ BEGIN ALTER TABLE public.members ADD COLUMN dietary_preference TEXT DEFAULT 'both';
EXCEPTION WHEN duplicate_column THEN
...
-- custom plan type
DO $$ BEGIN ALTER TYPE public.plan_type ADD VALUE IF NOT EXISTS 'custom';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_members_portal_username ON public.members(owner_id, portal_username) WHERE portal_username IS NOT NULL;

-- Backfill portal credentials for existing members
...
-- =====================================================
-- DELIVERY_AREAS — Zone perimeter columns
-- =====================================================
...
-- Super Admin platform-wide API config
...
-- Indexes
CREATE UNIQUE INDEX IF NOT EXISTS idx_profiles_business_slug ON public.profiles(business_slug) WHERE business_slug IS NOT NULL;

-- Auto-generate slugs for existing profiles
...
-- =====================================================
-- RLS POLICIES + GRANTS (all tables)
-- =====================================================
CREATE OR REPLACE FUNCTION _mf_drop_policies(tbl TEXT) RETURNS void AS $$
DECLARE pol RECORD;
BEGIN
...
SELECT _mf_drop_policies('delivery_batches');
...
SELECT _mf_drop_policies('batch_deliveries');
...
DO $$ BEGIN EXECUTE 'GRANT ALL ON public.invoices TO authenticated'; EXCEPTION WHEN undefined_table THEN NULL; END $$;
DO $$ BEGIN EXECUTE 'GRANT ALL ON public.invoice_items TO authenticated'; EXCEPTION WHEN undefined_table THEN NULL; END $$;
