-- ============================================================================
-- MULTIMODE ACTIVATION SYSTEM - FIXED PRODUCTION SCHEMA
-- Manages which modes are available to each tenant with upgrade pricing
-- ============================================================================

-- CRITICAL: Drop existing first (if needed)
DROP SCHEMA IF EXISTS mode_activation CASCADE;

-- Create schema FIRST (before any tables)
...
CREATE TABLE mode_activation.tenant_mode_access (
  ...
  plan_id UUID REFERENCES mode_activation.subscription_plans(id) ON DELETE SET NULL,
...
-- ============================================================================
-- Table: locked_modes - UPGRADE PRICING (FIX: Add ON DELETE CASCADE)
-- ============================================================================
...
CREATE TABLE mode_activation.locked_modes (
  ...
  upgrade_frequency VARCHAR(50) DEFAULT 'monthly',
...
('Pro - All Modes', 'Full access to all modes', 99.99, 'monthly', 5, ARRAY['mess', 'restaurant', 'canteen'], '{"support": "priority", "api": true}'),
('Enterprise', 'Custom pricing', 0, 'yearly', NULL, ARRAY['mess', 'restaurant', 'canteen'], '{"custom": true, "support": "24/7"}')
...
CREATE POLICY "Anyone can view subscription plans"
  ON mode_activation.subscription_plans FOR SELECT USING (true);
