-- ============================================================================
-- MESSFLOW RESTAURANT TABLE LOCKING FEATURE - PRODUCTION SCHEMA
-- PostgreSQL / Supabase
-- ============================================================================
...
-- restaurants.venues - Restaurant/Venue information
...
-- restaurants.venue_staff - Staff members per venue
...
-- restaurants.tables - Restaurant tables
...
-- Query: Is table X locked right now?
CREATE INDEX IF NOT EXISTS idx_locks_active 
  ON restaurants.table_locks(table_id) 
  WHERE expires_at > NOW();

-- Query: Get all active locks for admin venue view
CREATE INDEX IF NOT EXISTS idx_locks_venue 
  ON restaurants.table_locks(venue_id);

-- Query: Cleanup expired locks
CREATE INDEX IF NOT EXISTS idx_locks_expires 
  ON restaurants.table_locks(expires_at);

-- Query: Table-specific lock queries
CREATE INDEX IF NOT EXISTS idx_locks_table 
  ON restaurants.table_locks(table_id);

-- Other useful indexes
CREATE INDEX IF NOT EXISTS idx_tables_venue 
  ON restaurants.tables(venue_id);

CREATE INDEX IF NOT EXISTS idx_tables_status 
  ON restaurants.tables(venue_id, status);

CREATE INDEX IF NOT EXISTS idx_venue_staff_venue 
  ON restaurants.venue_staff(venue_id);

CREATE INDEX IF NOT EXISTS idx_venue_staff_user 
  ON restaurants.venue_staff(user_id);
...
-- VENUE_STAFF: Users see staff for their venues
...
-- TABLE_LOCKS: Users see locks for their venues
